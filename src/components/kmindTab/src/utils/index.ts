...
// import { usePublicStoreWithOut } from '/src/components/kmindTab/src/store/modules/public';
...
export const getWidgetBlockInfo = (): IWidgetBlockAttr => {
  ...
  const id = blockNode?.getAttribute('data-node-id') || '20230302162223-3rxpzda'
  ...
  const dataAssets = blockNode?.getAttribute('custom-data-assets') || ''
...
export const isMultiTree = (data: any) => {
  ...
  if (typeof data !== 'object' || typeof data?.data !== 'object' || !data.hasOwnProperty('children')) {
    return false
  }
...
export const isClickRemarkIcon = (e: any) => {
  ...
  return (
    e.target?.nextElementSibling?.attributes?.['p-id']?.nodeValue === '8792' ||
    e.target.attributes?.['p-id']?.nodeValue === '8793'
  )
}
...
// 还会有很小的概率点到a
export const isClickLinkIcon = (e: any): { isLink: boolean; linkUrl?: string } => {
  ...
  if (e.target?.nextElementSibling?.attributes?.['p-id']?.nodeValue === '7982') {
    // 点击到rect的情况：rect的nextElementSibling的p-id为7982
    isLink = true
    linkUrl = e.target.parentNode.getAttribute('href')
    ...
    isLink = true
    linkUrl = e.target.parentNode.parentNode.getAttribute('href')
  } else if (e.target?.childNodes[1]?.attributes?.['p-id']?.nodeValue === '7982') {
    // 点击到a标签的情况
    isLink = true
