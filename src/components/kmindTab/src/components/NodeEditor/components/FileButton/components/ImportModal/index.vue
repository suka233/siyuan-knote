...
<template>
  <a-modal v-model:visible="visible" title="导入至Kmind" @ok="handleOk">
    <div>
      <a-upload-dragger v-model:file-list="fileList" :max-count="1" :before-upload="handleBeforeUpload">
        <p class="ant-upload-drag-icon">
          <inbox-outlined />
        </p>
        <p class="ant-upload-text">点击或者拖拽文件至此处以导入</p>
        <p class="ant-upload-hint">支持文件格式：.kmind, .json, .xmind, .xlsx, .md</p>
      </a-upload-dragger>
    </div>
  </a-modal>
</template>
...
import { fileToBuffer } from '@/components/kmindTab/src/utils'
...
const mindFile = computed<File | undefined>(() => fileList.value?.[0]?.originFileObj)
...
const { loadKmindFullData } = useKmind(undefined, name!)
const handleJson = () => {
...
const handleXmind = async () => {
  await xmind
    ...
    .then((data) => {
      // console.log(data)
      kmind[name!].setFullData({ root: data })
      message.success('导入成功')
    })
    .catch(() => {
...
const handleExcel = async () => {
  try {
    ...
    kmind[name!].setFullData(layers[0][0])
    message.success('导入成功')
  } catch (error) {
    console.log(error)
    message.error('文件解析失败')
