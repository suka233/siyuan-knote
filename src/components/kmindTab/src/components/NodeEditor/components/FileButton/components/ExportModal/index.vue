导出
<template>
  <a-modal v-model:visible="visible" title="导出" :width="800" @ok="handleOk" :confirm-loading="loading">
    <div>
      <a-form>
        <a-row :gutter="[8, 8]">
          <a-col :span="24">
          ...
          <!--          </a-col>-->
          <a-col :span="6" v-show="fileType === 'png' || fileType === 'svg'">
            <a-form-item label="背景是否透明"><a-checkbox v-model:checked="isTransparent" /></a-form-item>
          </a-col>
          <a-col :span="6" v-show="fileType === 'kmind' || fileType === 'json'">
            <a-form-item label="是否导出主题，结构"><a-checkbox v-model:checked="isFullExport" /></a-form-item>
          </a-col>
          <a-col :span="24"> 请选择导出格式: </a-col>
          <a-col :span="8" v-for="item in exportTypeList" :key="item.type"
            ><file-type-list :item="item" @click="handleChangeType(item.type)" :active="fileType === item.type"
          /></a-col>
          <a-col :span="24" v-show="fileType === 'kmind'"
            ><a-alert
              ...
              show-icon
            >
              <template #icon><smile-outlined /></template> </a-alert
          ></a-col>
...
import { exportTypeList } from '@/components/kmindTab/src/config'
import FileTypeList from './components/FileTypeList/index.vue'
import { message } from 'ant-design-vue'
import { kmind, useKmind } from '@/components/kmindTab/src/hooks/useKmind'
...
const fileName = ref<string>(name as string)
...
const handleOk = async () => {
  ...
  switch (fileType.value) {
    ...
    case 'json':
      ...
      await kmind[name!].doExport.json('', isFullExport.value).then((_data) => {
        data = _data
      })
      break
    case 'png':
      await kmind[name!].doExport.png('', isTransparent.value).then((_data) => {
        data = _data
      })
      break
    case 'svg':
      await kmind[name!].doExport
        .svg(
          '',
          false,
          `* {
      ...
      break
    case 'xmind':
      await kmind[name!].doExport.xmind('').then((_data) => {
        data = _data
      })
      break
    case 'md':
      await kmind[name!].doExport.md().then((_data) => {
        data = _data
      })
      break
  }
