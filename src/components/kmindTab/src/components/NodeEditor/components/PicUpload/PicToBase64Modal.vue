<template>
  <a-modal v-model:visible="visible" title="插入图片" @ok="handleOk" :width="800">
    <!--        <a-row v-show="picUrl">-->
    ...
    <a-input addon-before="请输入图片标题" v-model:value="picTitle" v-show="picUrl" class="w-4/5" allow-clear />
...
// 处理图片上传
import { LoadingOutlined, FileImageTwoTone, ClearOutlined } from '@ant-design/icons-vue'
...
const handlePicBeforeUpload = (file: UploadProps['fileList'][number]) => {
  ...
  if (!isJpgOrPng) {
    message.error('仅支持上传图片格式文件!(jpeg/png/jpg/gif/svg/webp/bmp/tiff)')
    picFile.value = null
  }
