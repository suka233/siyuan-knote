  ...
  <a-dropdown>
    <template #overlay>
      <a-menu @click="handleMenuClick">
        <a-menu-item key="设置导图为测试数据"> 设置导图为测试数据 </a-menu-item>
        <a-menu-item key="console导图数据"> console导图数据 </a-menu-item>
        <a-menu-item key="RESET_LAYOUT"> RESET_LAYOUT </a-menu-item>
        <a-menu-item key="重载导图数据"> 重载导图数据 </a-menu-item>
        <a-menu-item key="打印activeNodeList"> 打印activeNodeList </a-menu-item>
        <a-menu-item key="调用插件打开浮窗">调用插件打开浮窗</a-menu-item>
...
const handleMenuClick = (e: any) => {
  switch (e.key) {
    case '设置导图为测试数据':
      kmind[name].setFullData(Object.assign({}, testData, { layout: undefined }))
      break
    case 'console导图数据':
      console.log('node', node.value)
      console.log('kmind', kmind)
      console.log('allData', JSON.stringify({ suka: kmind[name].getData(true) }))
      break
    case 'RESET_LAYOUT':
      kmind[name].execCommand('RESET_LAYOUT')
      break
    case '重载导图数据':
      kmind[name].setFullData(Object.assign({}, mindMapData.value, { layout: undefined }))
