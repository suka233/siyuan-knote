<template>
  <div :class="`animate-animated animate-slideInDown editor-card`">
    ...
    <span class="node-editor-btn pr-6">
      <back-forward />
      <node-editor-btn class="mr-2" :disabled="!node" @click="handleShowRichEditor('node')" />
      <tags-editor-btn :disabled="!node" class="mr-2" @click="showTagsEditor = true" />
      <hyper-link-btn :disabled="!node" class="mr-2" @click="showLinkEditor = true" />
      <remark-note-btn :disabled="!node" class="mr-2" @click="handleShowRichEditor('note')" />
      <pic-upload-btn :disabled="!node" class="mr-2" @click="showPic = true" />
    ...
    <!--      功能实现区-->
    <div>
      <!--      <remark-note-editor v-model:visible="showRichEditor" :node="node" :type="richEditorType" v-if="showRichEditor" />-->
      <!--      <rich-editor v-if="showRichEditor" v-model:visible="showRichEditor" :node="node" :type="richEditorType" />-->
      <rich-editor v-model:visible="showRichEditor" :node="node" :type="richEditorType" />
...
import ExportFile from '/src/components/kmindTab/src/components/NodeEditor/components/ExportFile/ExportFile.vue'
...
import RemarkNoteEditor from './components/RemarkNote/RemarkNoteEditor.vue'
const name = inject('name')
const publicStore = creatNamedPublicStore(name)()
const { node, activeNodeList } = toRefs(publicStore)
